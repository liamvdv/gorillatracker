...
def year_filter(query: Select[tuple[Video]], year: int) -> Select[tuple[Video]]:
    """Filter the query to return videos from a specific year."""
    return query.where(extract("year", Video.start_time) == year)


def daytime_filter(query: Select[tuple[Video]], start_hour: int, end_hour: int) -> Select[tuple[Video]]:
    """Filter the query to return videos from a specific time of the day."""
    return query.where(
        and_(start_hour <= extract("hour", Video.start_time), extract("hour", Video.start_time) <= end_hour)
    )


def month_filter(query: Select[tuple[Video]], start_month: int, end_month: int) -> Select[tuple[Video]]:
    """Filter the query to return videos from specific months."""
    return query.where(
        and_(start_month <= extract("month", Video.start_time), extract("month", Video.start_time) <= end_month)
    )
