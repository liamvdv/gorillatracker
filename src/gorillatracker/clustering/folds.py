...
class DistinctClassFold:
    ...
    def split(self, df: pd.DataFrame, label_column: str = "label_string", label_mask="new", unique_percentage=0.2, normalize_label_distribution=False):
        # Shuffle DF
        shuffled = df.sample(frac=1, random_state=self.random).reset_index(drop=True) if self.shuffle else df   
        
        # TODO(liamvdv): add warning if unevently distributed labels
