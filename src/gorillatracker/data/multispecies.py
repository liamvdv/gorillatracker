...
from gorillatracker.data.contrastive_sampler import ContrastiveSampler, ContrastiveImage
...
from gorillatracker.data.nlet import Nlet, FlatNlet
...
class MultiSpeciesContrastiveSampler(ContrastiveSampler):
    ...
    def negative(self, sample: ContrastiveImage) -> ContrastiveImage:
        ...
        sample_origin = self.ds.loc[sample.id, "origin"]
    ...
    def negative_classes(self, class_label: Label) -> list[Label]:
        return [label for label in self.contrastive_sampler.class_labels if label != class_label]
...
class MultiSpeciesSupervisedDataset(NletDataset):
    ...
    def _get_item(self, idx: Label) -> gtypes.Tuple[gtypes.Tuple[Id] | gtypes.Tuple[Tensor] | gtypes.Tuple[Label]]:
    ...
    def class_distribution(self) -> dict[Label, int]:
        return self.contrastive_sampler.ds["label"].value_counts().to_dict()
    ...
    def _crop_if_necessary(self, img: ContrastiveImage) -> Image.Image:
        ...
        if "bbox" in self.contrastive_sampler.ds.columns and isinstance(self.contrastive_sampler.ds.loc[img.id, "bbox"], list):
            bbox = self.contrastive_sampler.ds.loc[img.id, "bbox"]
            x, y, w, h = bbox
            bbox = (x, y, x + w, y + h)
    ...
    def __len__(self) -> Label:
        if self.partition == "val":
            return min(10000, len(self.contrastive_sampler.ds))