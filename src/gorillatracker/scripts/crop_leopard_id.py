...
def extract_image_file_names(images: list) -> dict[str, str]:  # type: ignore
...
def crop_image(image: cv2.Mat, bbox: list) -> cv2.Mat:  # type: ignore
    x, y, width, height = bbox
    return image[int(y) : int(y + height), int(x) : int(x + width)]


def filter_duplicate_annotations(annotations: list) -> list:  # type: ignore
