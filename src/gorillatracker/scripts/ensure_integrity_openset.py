    ...
    for image_file in image_files:
        ...
        if os.path.exists(bbox_path):
            ...
            for i in range(0, len(bbox_data), 5):
                index, x, y, w, h = map(float, bbox_data[i:(i + 5)]) # x,y is the center of the bbox and w,h are the width and height
    ...
    for image_file in image_files:
        ...
        if os.path.exists(bbox_path):
            ...
            if not seen_actual_subject:
                print(f"Warning: Actual subject '{actual_subject}' not found in bounding box file '{bbox_path}' for image '{image_file}'")
